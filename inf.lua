...
function utility:UDim2ToVector2(udim2, vector2)
    -- ЗАЩИТА ОТ NIL
    if not udim2 then return Vector2.new(0, 0) end
    if not vector2 or typeof(vector2) ~= "Vector2" then 
...
function library:CreateSettingsTab(menu)
    ...
    creditsSection:AddSeparator({text = 'bob?'});
    creditsSection:AddText({text = "gang"})
    creditsSection:AddSeparator({text = 'ChatGPT Solo'});
